/**
 * jni.cpp implements the bridge between the Java Minecraft plugin and the C++ detection engine
 */
...
namespace{
    ...
    // Engine instance given to JNI
    struct Entry{
        std::thread::id owner; // C++ thread ID (not windows TID) of the thread that owns this engine instance
        ac::Engine engine;     // C++ detection engine
        explicit Entry(ac::EngineSetup setup) : owner(std::this_thread::get_id()), engine(setup.trace, std::move(setup.factories)){}
    };
    ...
    Entry& get(jlong handle){
        ...
        if(it == registry.end())
            throw std::logic_error("Closed or invalid engine handle");
...
Java_dev_fox_anticheat_bridge_NativeBridge_nCreate(JNIEnv* env, jclass, jstring configuration){
    try{
        ...
        raise(env, "java/lang/IllegalStateException", e.what());
    }
    catch(...){
        raise(env, "java/lang/IllegalStateException", "Unknown native create error");
    }
...
Java_dev_fox_anticheat_bridge_NativeBridge_nDestroy(JNIEnv* env, jclass, jlong handle){
    try{
        ...
        raise(env, "java/lang/IllegalStateException", "Unknown native destroy error");
    }
}
...
Java_dev_fox_anticheat_bridge_NativeBridge_nSubmit(JNIEnv* env, jclass, jlong handle, jobject buffer, jint size){
    try{
        if(!buffer || size < 0 || size > 8192){
            throw std::invalid_argument("Invalid event size");
        }
        ...
        if(!bytes || capacity < size){
            throw std::invalid_argument("Expected a sufficiently large direct buffer");
        }
        ...
        raise(env, "java/lang/IllegalStateException", "Unknown native submit error");
    }
